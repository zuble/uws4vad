...
class Metrics(object):
    def __init__(self, cfg, mode, vis):
        ...
        self.mtrc_vis_table = cfg_vldt.get('MTRC_VISTABLE', False)
    ...
    ## https://scikit-learn.org/stable/modules/classes.html#module-sklearn.metrics
    def get_vl(self, labels, predictions, printt=True, plot=False, save_plot=False):
        
        assert len(labels) == len(predictions), f'len GT{len(labels)} != len VL {len(predictions)}'
        
        predictions_binary = [1 if p >= 0.5 else 0 for p in predictions]
        res_dict = {}

        if plot: self.plot_cm(str(self.net_name),labels, predictions_binary, save=save_plot)
        
        tn, fp, fn, tp = confusion_matrix(labels, predictions_binary).ravel()
        res_dict["TN"] = tn; res_dict["FP"] = fp
        res_dict["FN"] = fn; res_dict["TP"] = tp
        
        res_dict["ACCURACY"] = accuracy_score(labels, predictions_binary)
        ## (%% of True 1 out of all Positive predicted)
        res_dict["PRECISION"] = precision_score(labels, predictions_binary)
        ## (%% of True Positive out of all actual anomalies)
        res_dict["RECALL"] = recall_score(labels, predictions_binary)

        #self.vis.plot_lines('pr_auc', pr_auc)
        #self.vis.plot_lines('auc', rec_auc)
        #self.vis.lines('scores', predictions)
        #self.vis.lines('roc', tpr, fpr)
        
        ## PRECISION // RECALL
        ## https://glassboxmedicine.com/2019/03/02/measuring-performance-auprc/
        precision, recall, thresholds = precision_recall_curve(labels, predictions)
        res_dict["AU_PRC"] = auc(recall, precision)
        
        res_dict["AP"] = average_precision_score(labels, predictions)
        
        ## AreaUnderCurve_ReceiverOperatingCharacteristic
        ## TPR(hit) // FPR(miss)
        fpr, tpr, thresholds = roc_curve(labels, predictions)
        res_dict["AUC_ROC1"] = auc(fpr, tpr)

        res_dict["AUC_ROC2"] = roc_auc_score(labels, predictions)

        f1_res1 = f1_score(labels, predictions_binary)
        epsilon = 1e-08
        f1_res2 = 2*((pre_res*rec_res)/(pre_res+rec_res+epsilon))
        res_dict["F1_SCORE"] = f1_res1
        res_dict["F1_SCORE2"] = f1_res2
        
        log.info(f"VL")
        log.info(f"{res_dict = }")
        #for metric, value in res_dict.items(): log.info(f"{metric}: {value:.4f}")
        #return res_dict
    ...
    def tabler(self, table, name):
        ...
        try:
            if self.mtrc_save_table:
                plt.savefig( osp.join(self.cfg.EXPERIMENTPATH,f'{name}.png'), dpi=300, bbox_inches='tight', facecolor='black')
            if self.mtrc_vis_table: ## send table_img to vis
                ...
                #plt.imsave(osp.join(self.cfg.EXPERIMENTPATH,'table_np.png'), table_img)
