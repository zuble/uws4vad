...
def load_model(model_file_path="", model_name="mn30_as", mode=("b5", "b11", "b13", "b15"),
            feature_pooling_fn=feature_pooling.avg_time_pool):
...
def get_basic_model(model_name="mn30_as", feature_pooling_fn=feature_pooling.avg_time_pool, **kwargs):
    ...
    model = MNHearWrapper(mel=mel, net=net, scene_embedding_size=8448, timestamp_embedding_size=8448, **kwargs)
    return model
