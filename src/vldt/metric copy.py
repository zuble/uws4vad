...
class MetricCalculator:
    def calc_metrics(self, gt, preds, key, mtrc_info, curv_info=None, xtra_mtrcs=False):
...
class Metrics(object):
    def __init__(self, cfg_vldt, vis=None):
        ...
        ## always enabled durinf test
        ## 4 train it should be controlled by train loop
        ## either to plot the best or all 
        ## therefore return always both
        ## 
    ...
    def get_fl(self, vldt_info):
        ...
        ## further return from valditae to train and test
        ## if train save only for the best metric, and plot at train end
        ## reuse t
        return self.mtrc_info, self.curv_info 
    ...
    def get_fl_lbl(self, dict_data):
        ...
        for i, (lbl, data) in enumerate(dict_data.items()):
            ...
            if not i: ## label == '000.NORM'
                self.calculator.calc_far(data['GT'], data['FL'], lbl, self.mtrc_info)
            else:
                self.calculator.calc_metrics(data['GT'], data['FL'], lbl, 
                                            self.mtrc_info, self.curv_info, self.xtra_mtrcs)
    ...
    def get_fl_vid(self, dict_data):
        ...
        for i, (lbl, data) in enumerate(dict_data.items()):
            for fn, gt, fl in zip( data['FN'] , data['GT'] , data['FL']):
                ...
                if not i: ## 000.NORM
                    self.calculator.calc_far(data['GT'], data['FL'], lbl, self.mtrc_info)
                else: ## populate same idx with mtrc values
                    self.calculator.calc_metrics(data['GT'], data['FL'], lbl, 
                                            self.mtrc_info, self.curv_info, self.xtra_mtrcs) 
    ...
    def plot_curves(self, curv_info):
        ...
        pr_fig.update_layout(height=500, showlegend=True, title_text="PR Curves" )
    ...
    def plot_metrics(self, mtrc_info):
        ...
        for i, (metric_name, values) in enumerate(metrics_data.items()):
            if metric_name == 'FAR':
                ...
                opts = dict(
                    title=f'{str(metric_name)}',
                    xtickvals=[1],  # Only one tick for FAR
                    xticklabels=['000.NORM']
                )
            else:
