...
class Network_V(nn.Module):
    ...
    def forward(self, f, seq_len):
        ...
        mil_vls = self.clas(sls, seq_len) ## b VL
        return {
            'mil_vls': mil_vls
            }
...
#f_v = torch.cat((f_v, torch.zeros(2,20,1024)), dim=1)#print(f_v.shape)
seq_len = torch.sum(torch.max(torch.abs(f_v), dim=2)[0] > 0, 1)#print(seq_len)
#print(f_v[:, :torch.max(seq_len), :].shape)

#mil_vls, a_sls, v_sls, av_sls, audio_rep, visual_rep = net_av(f_a, f_v, seq_len)

mil_vls = net_v(f_v, seq_len) 
#loss_v = nn.BCELoss(mil_vls, label) ## bce
